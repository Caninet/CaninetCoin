...
#if defined(NDEBUG)
# error "Caninet cannot be compiled without assertions."
#endif
...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Caninet: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Caninet Signed Message:\n";
...
{
    // Caninet: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Caninet: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Caninet: Special short-term limits to avoid 10,000 BDB lock limit:
...
unsigned char pchMessageStart[4] = { 0xd7, 0xaa, 0x35, 0xb2 }; // Caninet: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CaninetMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CaninetMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CaninetMiner(CWallet *pwallet)
{
    printf("CaninetMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("caninet-miner");
    ...
    try { loop {
        ...
        printf("Running CaninetMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CaninetMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CaninetMiner, pwallet));
}
